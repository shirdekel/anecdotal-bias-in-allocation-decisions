...
##' @author Shir Dekel
##' @export
get_results_1 <- function(data = anecdotes1::data) {
  set_sum_contrasts()
  allocation_omnibus <-
    data %>%
  ...
  allocation_combined_high <-
    ...
    contrast("pairwise", adjust = "none") |>
    broom::tidy() |>
    filter(contrast == "combined_high - statistics_NA") |>
    summarise(
      effectsize::t_to_eta2(statistic, df)
    ) |>
    pull(Eta2_partial) |>
    printnum(digits = 3, gt1 = FALSE)

  allocation_combined_high_eta2 <-
    str_glue("$\\hat{{\\eta}}^2_p = {allocation_combined_high}$") |>
    set_names("allocation_combined_high_eta2")
